...
login=async(email,passward)=>{
if(email&&passward){
    try{
        const response=await firebase.auth().signInWithEmailAndPassword(email,passward)
        if(response){
    ...
    render(){
        return(
            ...
            <View>
                ...
                ,borderRadius:7}} onPress={()=>{this.login(this.state.emailId,this.state.passward)}}>
